...
if(isset($_GET['info'])){
    ...
    $sql = "SELECT 
        ...
        q.client_id,
        q.vehicle_number,
        GROUP_CONCAT(st.service_type_ids) AS service_type_ids
    FROM 
        (
        SELECT 
    ...
    WHERE 
        s.id = q.status_type AND q.id = '".$id."'";
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        <div class="form-row">
            <div class="form-group col-md-4 col-sm-6 col-lg-4 col-sm-6">
                ...
                <select name="service_type[]" multiple='multiple' class="form-control" id="service">
                    <?php
                        if (mysqli_num_rows($service_type) > 0) {
                            // Convert the service_type_ids string to an array
                            $selected_service_types = explode(',', $track['service_type_ids']);
                            
                            while ($servicetype = mysqli_fetch_assoc($service_type)) {
                                // Check if the current service type ID is in the selected service types array
                                $selected = in_array($servicetype['id'], $selected_service_types) ? 'selected' : '';
                                
                                echo '<option value="' . $servicetype["id"] . '" ' . $selected . '>' . $servicetype["type"] . '</option>';
                            }
                        }
                    ?>
                </select>
                ...
                <div class="col-8">
                    ...
                    if (mysqli_num_rows($status_type) > 0) {
                        while($statustype2 = mysqli_fetch_assoc($status_type)) {
                            ...
                            <?php 
                                echo ' name="status_type"
                                value="'.$statustype2['id'].'" type="radio" class="custom-control-input">
